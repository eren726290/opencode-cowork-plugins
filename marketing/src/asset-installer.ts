...
export function installMarketingAssets(packageRoot: string): { commandsInstalled: string[]; skillsInstalled: string[] } {
  const commandSource = path.join(packageRoot, "agents", "marketing", "commands")
  const skillSource = path.join(packageRoot, "agents", "marketing", "skills")

  const commandTarget = getOpenCodeCommandsDir()
  const skillTarget = path.join(getOpenCodeSkillsDir(), "marketing")
